...
def solve_simplex_model(params):
    ...
    A_ub = [
        ...
        [-params['cal1'], -params['cal2'], -params['cal3']]
    ]
    b_ub = [-params['min_y1'], -params['min_y2'], -params['min_y3'], -params['min_time'], -params['min_cal']]
...
class GraphiqueTab(tk.Frame):
    def __init__(self, parent):
        ...
        self.params_frame = ScrollableFrame(self.left_panel).scrollable_frame
...
class SimplexeTab(tk.Frame):
    def __init__(self, parent):
        ...
        self.tableaux_frame = ScrollableFrame(self.right_panel, bg_color="white").scrollable_frame
    ...
    def reset(self):
        ...
        for widget in self.tableaux_frame.winfo_children(): widget.destroy()

    def solve(self):
        try:
            p = self.get_params()
            y1_opt, y2_opt, y3_opt, w_opt = solve_simplex_model(p)
            ...
            final_matrix, final_basis = tableaux[-1]["matrix"], tableaux[-1]["basis"]
            dual_sol = [final_matrix[i, -1] if b_idx < 5 else 0.0 for i, b_idx in enumerate(final_basis)] + [0]*5
            
            self.lbl_dual.config(text=f"Dual: x1={dual_sol[0]:.2f}, x3={dual_sol[2]:.2f}, x4={dual_sol[3]:.2f}")
        except Exception as e:
            messagebox.showerror("Erreur", str(e))
